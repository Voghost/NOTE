...
* 特点
	1. 不修改参数位置不会变
	2. 元素从文档流中脱离
	3. 会改变元素性质,行内变成块，块的高度被内容撑开
	4. 绝对定位会是元素提高层级
* 参照物:相对于元素的 **包含块** ,且包含块要确定位置
* 包含块: 当前元素最近的最近的<b>块</b>元素
* 水平布局: 包含以下九个值(九个值的长度为父元素内容区宽度)
	1. left
	2. margin-left
	3. border-left
	4. padding-left
	...
	8. margin-right
	9. right
	* 备注(产生过度约束，如left:0;right:0;):
		1. 当九个值中没有auto则自动调整right值
		2. 如果有auto，则自动调整设为auto的值时等式满足
		3. 垂直方向类似
...
#### 3.4.5 元素层级
* 对于开启了定位的元素,可以通过 `z-index` 属性来制定元素的层级
	1. z-index 需要一个整数为参数，值越大元素的层级越高，越优先显示
